...
#matplotlib part
def plot_tf_scores(document_name, tf_df):
    tf_df = tf_df.sort_values(by='TF', ascending=False)
    plt.figure(figsize=(8, 5))
    plt.barh(tf_df['Word'], tf_df['TF'], color='skyblue')
    plt.xlabel('TF Score')
    plt.ylabel('Word')
    plt.title(f'Top TF Words for {document_name}')
    plt.gca().invert_yaxis()
    plt.show()

plot_tf_scores('Meditations', top_meditations_tf)
plot_tf_scores('Walden', top_walden_tf)
